...
export function TaxRateForm() {
  ...
  return (
    <>
      ...
      {open && <TaxModal key={nonce} onClose={() => setOpen(false)} onAddAnother={openFresh} />}
    </>
  );
}

function TaxModal({ onClose, onAddAnother }: { onClose: () => void; onAddAnother: () => void }) {
      ...
      <div className="w-full max-w-lg rounded-xl bg-white p-6 shadow-xl">
        ...
        {state.success ? (
          <div className="mt-6 rounded-lg bg-pine/10 p-4 text-sm text-pine">
            Tax rate created successfully. Add another or close to continue.
